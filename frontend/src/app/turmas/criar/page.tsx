...
export default function CriarEditarTurma() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      if (turmaSalva) {
        ...
        setIdEditando(turma.id); // Guardar o ID em modo edição
  ...
  const handleCriarTurma = async (data: TurmaFormData) => {
    try {
      const urlAcesso = idEditando ? `/api/turmas/${idEditando}` : "/api/turmas";
            ...
            <form onSubmit={handleSubmit(handleCriarTurma)} className="flex-1 w-full max-w-[500px] bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-gray-200 dark:border-slate-700">
              ...
              <div className="flex flex-col gap-5 w-full font-sans">
                ...
                <div className="flex w-full gap-4">
                  ...
                  <div className="flex-1 flex flex-col">
                    ...
                    <input 
                      type="text" 
                      {...register("codigo")}
                      className="w-full h-11 px-4 bg-gray-50 dark:bg-slate-900 border border-gray-300 dark:border-slate-600 rounded-lg text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500 transition-all text-sm font-mono uppercase" 
                  ...
                  <div className="w-[120px] shrink-0 flex flex-col">
                    ...
                    <input 
                      ...
                      min="1" 
                      {...register("capacidade")}
                      className="w-full h-11 px-4 bg-gray-50 dark:bg-slate-900 border border-gray-300 dark:border-slate-600 rounded-lg text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500 transition-all text-sm text-center" 
                    />
                ...
                <div className="p-4 bg-gray-50 dark:bg-slate-900/50 rounded-xl border border-gray-200 dark:border-slate-700">
                  ...
                  {(errors.horaInicio || errors.horaFim) && <span className="text-xs font-medium text-red-500 mt-2 block">Preencha os horários corretamente</span>}
                </div>
              </div>

              <div className="w-full flex gap-3 mt-8">
                <button type="submit" className="flex-1 h-11 bg-primary-600 hover:bg-primary-700 active:bg-primary-800 text-white rounded-lg font-sans font-medium text-sm transition-colors shadow-sm">
                  {idEditando ? "Atualizar Turma" : "Criar Turma"}
                </button>
                <button type="button" onClick={() => router.push("/turmas")} className="flex-1 h-11 bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700 active:bg-gray-100 rounded-lg font-sans font-medium text-sm transition-colors shadow-sm">
                  Cancelar
                </button>
              </div>
            </form>

            <div className="flex-1 w-full lg:max-w-sm bg-primary-50 dark:bg-primary-900/20 border border-primary-200 dark:border-primary-800/50 rounded-2xl p-6 shadow-sm flex flex-col">
              <h3 className="font-serif text-lg font-bold text-primary-900 dark:text-primary-100 mb-2">
                Gestão de Alunos
              </h3>
              <p className="font-sans text-sm text-primary-700 dark:text-primary-300 mb-6">
                Para adicionar ou remover alunos de uma turma, navegue até a seção dedicada de alunos. O controle de presenças é feito de forma separada.
              </p>
              <button
                type="button"
                onClick={() => router.push("/alunos")}
                className="w-full h-10 bg-white dark:bg-slate-800 text-primary-600 dark:text-primary-400 hover:bg-gray-50 dark:hover:bg-slate-700 border border-primary-200 dark:border-primary-800 rounded-lg font-sans font-medium text-sm transition-colors shadow-sm mt-auto"
              >
                Ir para Alunos
              </button>
            </div>
