...
* 📄 **`docker-compose.yml`**: O "maestro" da infraestrutura. Com um único comando (`docker compose up`), ele liga o banco de dados, o backend e o frontend de uma só vez na sua máquina.
* 📄 **`.env.example`**: O mapa do tesouro. Ele mostra quais variáveis de ambiente (senhas, links de API) você precisa ter na sua máquina para o projeto rodar.
